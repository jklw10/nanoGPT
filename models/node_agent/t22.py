...
def run_optuna_search():
    ...
    # Migrate old files if they exist in the root directory
    for old_file in ["hp_relations.json", "best_blueprint.json"]:
        if os.path.exists(old_file) and not os.path.exists(f"RL/{old_file}"):
            ...
            print(f"📁 Migrated {old_file} to RL/ directory.")
    ...
    engine = {
        "phase": "WARMUP",    # WARMUP -> IDLE <-> TESTING <-> SUSPENDING
        "tested_rule": None,
        "suspended_rule": None,
        "suspended_law": None,
        "start_gen": 0,
    ...
    while True:
        ...
        if engine["phase"] == "TESTING":
            strike_text = f" (Strikes: {engine['strikes']}/2)" if engine['strikes'] > 0 else ""
            status = f"🧪 PHASE 1: GAUNTLET '{engine['tested_rule']}' ({generation - engine['start_gen']}/{gen_count}){strike_text}"
        elif engine["phase"] == "SUSPENDING":
            status = f"🔪 PHASE 2: ABLATING '{engine['suspended_rule']}' ({generation - engine['start_gen']}/{gen_count})"
        else:
        ...
        # Track historical baselines ONLY when safely mapping (not testing/ablating)
        ...
        if study.best_value > global_best_fitness:
            ...
            # Create a permanent snapshot of this exact peak
            peak_file = f"RL/peak_gen{generation}_fit{global_best_fitness:.5f}.json"
            with open(peak_file, 'w') as f:
                json.dump(ckpt_data, f, indent=4)
        
        # --- THE GAUNTLET STRIKE SYSTEM (Mid-Phase 1 Catastrophe Check) ---
        if engine["phase"] == "TESTING":
            ...
            if len(historical_fits) > 0:
                ...
                if p_val < 0.01 and current_yield < (0.6 * scale_baselines[(N,D)]['yield_ema']):
                    ...
                    if engine['strikes'] >= 2:
                        ...
                        # Move to suspended pile instead of just yeeting
                        suspend_rule_to_file(engine['tested_rule'], relations[engine['tested_rule']], "Failed Gauntlet Strikes (Yield Dropped)")
        ...
        # =================================================================
        # THE CONTINUOUS SCIENTIFIC ENGINE (End of 9-Gen Cycle Evaluations)
        # =================================================================
        if generation >= (gen_count * 2) and generation % gen_count == 0:
            
            # --- EVALUATE PHASE 1: TESTING ---
            if engine["phase"] == "TESTING":
                if global_best_fitness > engine["start_best"]:
                    print(f"\n🎓 HYPOTHESIS ACCEPTED! Phase 1 Rule '{engine['tested_rule']}' pushed the global peak.")
                    engine["tested_rule"] = None
                    engine["phase"] = "IDLE"
                else:
                    # Rule survived Gauntlet but didn't push the peak. Transition to Phase 2.
                    available_to_suspend = [k for k in relations.keys() if k != engine["tested_rule"]]
                    if available_to_suspend:
                        engine["suspended_rule"] = random.choice(available_to_suspend)
                        ...
                        print(f"\n🤔 Rule '{engine['tested_rule']}' survived but didn't improve peak.")
                        print(f"   Initiating Phase 2: Suspending old rule '{engine['suspended_rule']}' to test landscape.")
                        save_relations(relations, RELATIONS_FILE)
                    else:
                        # No old rules to suspend! We just ablate the new one and drop to IDLE.
                        print(f"\n📉 Rule '{engine['tested_rule']}' didn't improve peak and no rules exist to suspend. Pruning.")
                        suspend_rule_to_file(engine["tested_rule"], relations[engine["tested_rule"]], "Failed to push peak, no others to ablate")
                        del relations[engine["tested_rule"]]
                        engine["tested_rule"] = None
                        engine["phase"] = "IDLE"
                        save_relations(relations, RELATIONS_FILE)

            # --- EVALUATE PHASE 2: SUSPENDING (Ablation) ---
            elif engine["phase"] == "SUSPENDING":
                ...
                if global_best_fitness > engine["start_best"] or avg_yield > (avg_ema * 1.05):
                    ...
                    suspend_rule_to_file(engine["suspended_rule"], engine["suspended_law"], "Ablation increased performance/yield")
                    
                    # The Phase 1 rule is officially kept!
                    engine["suspended_rule"] = None
                    engine["suspended_law"] = None
                    engine["tested_rule"] = None 
                    engine["phase"] = "IDLE"
                else:
                    print(f"\n🛡️ RULE VINDICATED! '{engine['suspended_rule']}' is structural. Reinstating it.")
                    relations[engine["suspended_rule"]] = engine["suspended_law"]
                    
                    # Phase 1 rule didn't help, and Ablating didn't help. Prune Phase 1.
                    if engine["tested_rule"] and engine["tested_rule"] in relations:
                        print(f"   Ablating Phase 1 rule '{engine['tested_rule']}' since it didn't help.")
                        suspend_rule_to_file(engine["tested_rule"], relations[engine["tested_rule"]], "Phase 1 rule failed Phase 2 suspension test")
                        del relations[engine["tested_rule"]]
            ...
            # --- IDLE TRANSITION: PROPOSE OR RANDOMLY ABLATE ---
            # (Note: This seamlessly chains off the evaluations above on the exact same generation tick)
            if engine["phase"] in ["IDLE", "WARMUP"]:
                ...
                proposed_rule = analyze_and_collapse_dimension(study, search_space_for_math, generation)
                
                if proposed_rule:
                    engine["tested_rule"] = proposed_rule['slave']
                    relations[engine["tested_rule"]] = proposed_rule.copy()
                    if 'slave' in relations[engine["tested_rule"]]:
                        del relations[engine["tested_rule"]]['slave']
                    
                    engine["phase"] = "TESTING"
                    engine["start_gen"] = generation
                    engine["start_best"] = global_best_fitness
                    engine["strikes"] = 0
                    print(f"\n💡 Initiating Phase 1: Testing new proposed rule for '{engine['tested_rule']}'")
                    save_relations(relations, RELATIONS_FILE)
                    
                elif len(relations) > 0:
                    # RANSAC has no ideas. Keep the engine moving by challenging a random rule!
                    ...
                    engine["yields"] =[]
                    engine["emas"] =[]
                    print(f"\n🧪 No new rules detected. Initiating Continuous Ablation on '{engine['suspended_rule']}'")
                    save_relations(relations, RELATIONS_FILE)
