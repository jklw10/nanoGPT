...
class PureTriadicBrain(nn.Module):
    def __init__(self, dim, batch_size, num_nodes=80, **hp_grids):
        ...
        self.register_buffer('Router', torch.zeros(batch_size, self.N, self.N, device=DEVICE))
    ...
    def step(self, eye_env, distance_env):
        self.step_counter += 1.0
        
        total_in = torch.bmm(self.Router, self.output)
        self.target = total_in / self.A_sums
        
        self.target[:, 0, :] = eye_env
        self.target[:, 1, :] = distance_env

        raw_pred = torch.einsum('bnj,bnjk->bnk', self.state, self.W1)
        
        # UNTANH applies to raw_pred (which is a 3D tensor)
        ...
        #E_curr = error_signal*error_signal# torch.norm(error_signal, dim=2, keepdim=True) 
        E_curr = F.softmax(error_signal,dim=-1)
        #E_curr = torch.sigmoid(error_signal)#,dim=-1)
        #E_curr = torch.abs(error_signal)#*error_signal
        #E_curr = utils.rms(error_signal)
        #E_curr = torch.tanh(torch.abs(error_signal))
        #E_curr = torch.sigmoid(error_signal)
        baseline_mask = (self.E_baseline == 0).float()
        self.E_baseline.copy_(self.E_baseline * (1 - baseline_mask) + E_curr * baseline_mask)
        
        # EMA_SPEED applies to E_curr (which is a 3D tensor)
...
if __name__ == "__main__":
    ...
    sweep = Sweep2D(
        ...
        param_y_name='LR', param_y_vals=np.linspace(0.0005, 0.05, 32)
    )
