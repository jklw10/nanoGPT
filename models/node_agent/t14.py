...
class PureTriadicBrain(nn.Module):
    def __init__(self, dim, batch_size, num_nodes=80, **hp_grids):
        ...
        hps = {
            ...
            "NOISE_SCALE": 0.5546767726839642,
    ...
    def step(self, eye_env, stomach_env):
        ...
        # 4. Route the signals
        self.target = torch.bmm(self.A, self.output)
        ...
        error_signal = prediction - self.target #todo check for sanity
...
if __name__ == "__main__":
    ...
    #)
    sweep = Sweep2D(
        param_x_name='UNTANH', param_x_vals=np.linspace(0.5, 1.6, 16),
        param_y_name='w_scale', param_y_vals=np.linspace(0.1, 1.4, 16)
    )
