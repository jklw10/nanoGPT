...
class TriadPiece(nn.Module):
    def __init__(self, dim, batch_size, num_nodes=10, **hp_grids):
        ...
        #self.register_buffer('weight',  torch.zeros(batch_size, num_nodes, self.D, self.D, device=DEVICE))
        #base_weight = torch.zeros(1, self.weight.shape[1], self.D, self.D, device=DEVICE) 
    ...
    def calibrate_w_scale(self):
        ...
        w_scale = self.hp('w_scale', 4)
        self.weight.data.copy_(utils.rms_norm(self.weight,dim=[-1,-2])*w_scale * self.dynamic_w_scale.view(-1, 1, 1, 1))
    ...
    def get_gradient(self, layer_input, error_signal):
        ...
        # 1. SANE ERROR TRACKING
        # Track the absolute error magnitude instead of using softmax
        #retry softmax, it was so much better in og version than abs.
        error_abs = error_signal.abs()
        self.E_baseline.mul_(1.0 - ema_speed).add_(ema_speed * error_abs.detach())
        
        # 2. PLASTICITY (Surprise-driven)
        # If current error is HIGHER than expected baseline, increase plasticity
        surprise = F.relu(error_abs - self.E_baseline)
        node_surprise = surprise.mean(dim=-1, keepdim=True).unsqueeze(-1) # (B, N, 1, 1)
        plasticity = 1.0 + (node_surprise * rew_sens)
        
        noisy_state = layer_input
        
        noisy_state = layer_input + torch.randn_like(layer_input) * n_scale
            
        # 3. FIX THE TRANSPOSED GRADIENT
        # Forward pass: 'bnj, bnjk -> bnk' (j is input, k is output)
        # Descent direction: - input_j * error_k
        grad = -torch.einsum('bnj, bnk -> bnjk', noisy_state, error_signal)
        
        # 4. SANE NORMALIZATION
        # rms_norm forces all gradients to length 1, destroying convergence.
        # Instead, normalize the direction, but scale by actual error magnitude and plasticity.
        err_mag = error_abs.mean(dim=-1, keepdim=True).unsqueeze(-1)
        grad_norm = grad.norm(dim=[-1, -2], keepdim=True) + 1e-8
        grad_dir = grad / grad_norm
        
        return grad_dir * err_mag * plasticity
...
class PureTriadicBrain(nn.Module):
    ...
    def step(self, env_input):
        ...
        new_A_ema = raw_A * is_first + (self.A_ema * self.c_moment + raw_A * (1.0 - self.c_moment)) * (1.0 - is_first)
...
def visualize_routing(sweep, fitness, brain):
    ...
    perf_grid = brain.A[best_idx].cpu().numpy()
    ax1 = plt.subplot(1, 3, 1)
    im1 = ax1.imshow(perf_grid, origin='lower', extent=sweep.get_extent(), aspect='auto', cmap='viridis', vmin=0, vmax=1.0)
    ax1.set_title("routing")
