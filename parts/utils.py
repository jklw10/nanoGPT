...
def sin_relu(x, ratio = 1.25):
     base = F.relu(x)
     sine_mod = F.relu(torch.sign(x)) * torch.sin(ratio * x)
     return base + sine_mod

def leaky_sin_relu_power(x, ratio = 0.9999, power=1.0, pow_max=2.5):
     base = F.leaky_relu(x)
     base_relu = F.relu(x)+1e-5
     #ratio = ratio / (power * (pow_max ** (power - 1)))
     sine_mod = F.relu(torch.sign(x)) * ratio * torch.sin(base_relu**power)
     return base + sine_mod
...
class Psinrelu(nn.Module):
    def __init__(self, n_embd, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.alpha = nn.Parameter(torch.ones(n_embd))
        self.pow = nn.Parameter(torch.ones(n_embd))
        #torch.nn.init.uniform_(self.alpha,0,1.0)
    def forward(self, x):
        slope = torch.sigmoid(self.alpha)*0.99999
        
        power = torch.sigmoid(self.pow)*2+0.5
